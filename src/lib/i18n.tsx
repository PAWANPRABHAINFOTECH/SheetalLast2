...
const translations: Record<Language, Record<string, string>> = {
  hi: {
    ...
    'nav.gallery': 'गैलरी',
    'nav.members': "'''Do not make any visual modifications. The phrases I write are commands to understand what I want, not to be written down. Understand their content well, then execute what is required.'''\n                                        \n                                            \n                                            Fix only the data connection between Admin Site Settings and the main website. Do not change the existing design, layout, colors, typography, spacing, navigation, images, or any other functionality.\n\nIn the Admin Panel → Site Settings, the fields “साइट का नाम” and “पता” are saving successfully, but their updated values are not reflected on the main/public website.\n\nPlease trace the existing Site Settings data flow and fix the binding so that:\n\nThe main website header/branding reads the site name directly from the existing Admin Site Settings data.\n\nWherever the address is displayed on the main website, it must read the address from the same existing Admin Site Settings data.\n\nWhen an administrator changes and saves the site name or address, the new values must automatically appear on the public website without manually editing the frontend code.\n\nDo not create a second settings system, duplicate fields, new database tables, or new configuration.\n\nReuse the existing Admin Site Settings storage/API/database already used by the project.\n\nCheck for hard-coded site name/address values in the public frontend and replace only those references with the existing Site Settings values.\n\nPreserve all existing Hindi/English language behavior.\n\nDo not modify any other Site Settings fields or functionality.\n\nDo not make any visual/design changes.\n\nAfter fixing, verify the complete flow: Admin changes value → Save → public website displays the updated value.\n\nImportant: This is a targeted bug fix only. Do not redesign or restructure anything. Do not change any unrelated code.",
    'nav.contact': 'संपर्क',
